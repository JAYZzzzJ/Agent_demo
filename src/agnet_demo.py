...
def extract_specs_from_stitched_image(stitched_img):
    ...
    prompt = """
    你是一个资深的电气自动化工程师。这是一张产品商详长图。
    请仔细分析图中的技术图纸、尺寸标注和参数表格。
    请用清晰、简练的中文，罗列出其中所有有价值的技术参数（如电压、孔径、材质、接线方式等）。
    如果没有找到明确的参数，请回复“无附加图纸参数”。
    """
    try:
...
# ==========================================
# 2. 升级版双流 RAG 系统 (Metadata 隔离 + DPO 日志)
# ==========================================
            ...
            try:
                ...
                self.text_collection.upsert(
                    ...
                    metadatas=[{
                        ...
                        "detailed_specs": image_specs_text
                    }],
                    ids=[f"txt_{product_id}"]
                )
                print("✅ 文本基础特征与参数 Metadata 入库成功 (已覆盖旧数据)")
